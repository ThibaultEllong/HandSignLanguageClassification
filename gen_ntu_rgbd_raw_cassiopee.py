...
############################################################################################################################################################
#
# This file is a custom version of the proposed script by the authors of the NTU RGB+D dataset. 
# It has been modified to generate a pickle file for the cassiopee dataset.
#  
############################################################################################################################################################
...
def pre_normalization(data, zaxis=[1, 2], xaxis=[3, 6]): # zaxis = pelvis, chestUpper, xaxis = rShldrBend, lShldrBend
    ...
    print('sub the center joint #1 (pelvis in cassiopee)')
    prog_bar = mmcv.ProgressBar(len(s))
    for i_s, skeleton in enumerate(s):
        if skeleton.sum() == 0:
            continue
    ...
    print('parallel the bone between pelvis(jpt 1) and '
          'chestUpper(jpt 2) of the first person to the z axis')
    prog_bar = mmcv.ProgressBar(len(s))
    for i_s, skeleton in enumerate(s):
        if skeleton.sum() == 0:
            continue
    ...
    print('parallel the bone between left shoulder(jpt 3) and '
          'right shoulder(jpt 6) of the first person to the x axis')
    prog_bar = mmcv.ProgressBar(len(s))
    for i_s, skeleton in enumerate(s):
        if skeleton.sum() == 0:
            continue
...
def read_skeleton_filter(file):
    with open(file, 'r') as f:
        skeleton_sequence = {}
        skeleton_sequence['num_frame'] = int(f.readline()) # 1st line is number of frames
            ...
            for m in range(frame_info['numBody']):
                ...
                body_info = {
                    ...
                    for k, v in zip(body_info_key,
                                    f.readline().split()) # reads the 3rd line
                }
                body_info['numJoint'] = int(f.readline()) #First line of each frame is number of joints
                body_info['jointInfo'] = []
                for v in range(body_info['numJoint']):
                    joint_info_key = [
                        'x', 'y', 'z' #reads the 3 coordinates of each joint
                    ]
                    joint_info = {
                        k: float(v)
