...
def main():
    ...
    dataset.video_infos = dataset.video_infos[0:3]
    ...
    ntuClassesPath = '../tools/data/skeleton/label_map_ntu120.txt'
    ntuClasses = [x.strip() for x in open(ntuClassesPath).readlines()]

    predictions = numpy.argmax(outputs, axis=1)

    for i in range(len(predictions)):
        label = ntuClasses[dataset.video_infos[i]['label']]
        predictedAction = ntuClasses[predictions[i]]
        print("Video {} - Real Action = {}   ;  Predicted Action = {}".format(i, label, predictedAction))
